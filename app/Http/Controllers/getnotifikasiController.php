...
class getnotifikasiController extends Controller
{
  ...
  public function set(){
    $get = DB::table('d_notification')->where('n_name', 'LIKE', '%notifikasi%')->get();
  ...
  public function cek($name, $qty, $link){
    $cek = DB::table('d_notification')->where('n_name', 'LIKE', '%notifikasi%')->where('n_name', $name)->first();
    // dd(count($date));
    if (count($cek) != 0) {
  ...
  public function gettmpnotif(){
    $cek = DB::table('d_notification')->where('n_name', 'LIKE', '%notifikasi%')->get();
