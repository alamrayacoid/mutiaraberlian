...
class PembayaranNotaController extends Controller
{
    ...
    public function getTermin(Request $request)
    {
        ...
        $data = DB::table('d_productionorderpayment')
            ...
            ->select('pop_termin', DB::raw('date_format(pop_datetop, "%d-%m-%Y") as pop_datetop'), 'pop_value', DB::raw('replace(format(round(pop_value - pop_pay),0), ",", ".") as pop_value'), DB::raw('replace(format(round(pop_value - pop_pay),0), ",", "") as pop_valueint'), 'pop_value as tagihan', DB::raw('replace(format(round(pop_value),0), ",", "") as tagihanrp'))
            ->where('po_nota', '=', $nota)
            ->where('pop_status', '=', 'N')
            ->get();
